...
public class AddEventActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(getIntent().getExtras()!=null){
			...
			String desString = getIntent().getStringExtra("DES");
			
			if(!desString.contains("nothing"))
			eventDescription.setText(desString);
			
			String timeString = getIntent().getStringExtra("TIME");
			
			if(timeString.contains(" ")){
			
			if(!timeString.contains("nothing")){
			Toast.makeText(Localendar.instance, "add to my calendar!", Toast.LENGTH_SHORT).show();
			String bigPart[] = timeString.split(" ");
			String dateString[] = bigPart[0].split("/");
			...
			String timeStrings[] = bigPart[1].split(":");
			int hours = Integer.parseInt(timeStrings[0]);
			int minutes = Integer.parseInt(timeStrings[1]);
			
			eventDate.updateDate(year, month, day);
			eventTime.setCurrentHour(hours);
			eventTime.setCurrentMinute(minutes);
			}
			}
